...
console.log(findLP)
    if (findLP) {
        ...
        if (findLP.lpNumber === lpNumber && !isNaN(ValidOutstandingLoan) && ValidOutstandingLoan > 0) {
            ...
            deductionNewHTML += `<div class="matChDeductionToLoan" id="${lpNumber}">
                <div class="matchDeductionLp"><span>LP${lpNumber}</span><span >${findLP.name}</span></div>
                <div class="outstandinLoan">Outstanding Normal Loan: ${toNairaCurrency(ValidOutstandingLoan)}</div>
                <dialog id="pay-dialog" class="paydialog">
                <p>Are you sure you want to confirm payment for this member</p>
                <div class="payButtons"><button class="cancelPayment" id="cancel-payment">Cancel</button><button class="confirmPayment" id="confirm-payment">Confirm</button></div>
                </dialog>
                <div class="amountToPay">Amount to Pay: ${toNairaCurrency(LoanDeduction)}<button type="button" class="pay" data-user-lp="${lpNumber}"">Pay</button></div>
            </div>`;
        }
